...
jax.config.enable_checks = False
...
class AllGatherTest(jtu.JaxTestCase):
  ...
  def test_test(self):
    np.testing.assert_equal(8, jax.device_count())
  ...
  def test_right_permute_example(self):
    ...
    def right_permute_kernel(input_ref, output_ref, send_sem, recv_sem):
      ...
      left_neighbor = lax.rem(my_id + num_devices - 1, num_devices)
    ...
    print('xla_results.shape', xla_result.shape,
          'pallas_result.shape', pallas_result.shape)
    np.testing.assert_allclose(xla_result[:, ::128], pallas_result[:, ::128])
