...
def setUpModule():
  _exit_stack.enter_context(jtu.set_host_platform_device_count(4))
...
class InterpretTest(jtu.JaxTestCase):
  ...
  def test_XXX(self):
    ...
    input_arr = jnp.arange(
      outer_block_size[0] * num_devices * outer_block_size[1] * num_devices,
      dtype=jnp.float32
    ).reshape(outer_block_size[0] * num_devices, outer_block_size[1] * num_devices)
    input_arr = jax.device_put(input_arr, sharding)
